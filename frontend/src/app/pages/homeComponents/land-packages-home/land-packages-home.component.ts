...
@Component({
  selector: 'app-land-packages-home',
  templateUrl: './land-packages-home.component.html',
  styleUrls: ['./land-packages-home.component.scss']
})
export class LandPackagesHomeComponent {
  packages: any[] = [];
    ...
    this.service.packagesFetch().subscribe({
      next: (res) => {
        if (res?.data?.rows) {
          ...
          this.packages = res.data.rows.filter((pkg: { name: string; }) => pkg.name === 'Land Package').slice(0, 3);
        } else {
